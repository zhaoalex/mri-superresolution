...
def nii_to_png(input_path, output_path):
    ...
    for files in file_list:
        if files.lower().endswith('.dcm'):
            image = pydicom.dcmread(os.path.join(input_path, files))
            img_name = files.replace('.dcm', '.png')
            cv2.imwrite(os.path.join(output_path, img_name), image.pixel_array)

        if files.lower().endswith(('.nii', '.nii.gz')):
            pixel_array = nibabel.load(os.path.join(input_path, files)).get_fdata()
